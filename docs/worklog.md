...
### GET / の一覧データをメモリキャッシュ（DB往復を0に）
- GET / は最ホットの動的エンドポイントで posts 一覧 + comments の 2 往復をしていた。
  `RwLock<Option<Vec<Post>>>` に一覧（csrf 空）をキャッシュし、ヒット時は **DB 往復 0**
  （current_user/csrf/flash は全てメモリ）。csrf は描画時にセッション毎に差し込む。
- 無効化（None セット・lazy 再構築）: POST / ・POST /comment ・POST /admin/banned ・/initialize。
  手動検証: index 先頭投稿にコメント→ GET / の comment_count が即 11→12 に反映（整合性 OK）。
- 実行中 mysqld CPU が 70% → **30〜45%** に低下（キャッシュヒットで DB 負荷減を確認）。

### ⚠️ 共有ベンチマーカーの混雑が支配的（セッション終盤）
- 14:20 以降、同時実行が **11〜16 台**に達し、全チームのスコアが圧縮（自分も 320k〜342k）。
- index-cache ラン: `pass:true / success 421,445 / fail 6,594`。失敗は
  「リクエストに失敗しました (GET / , /@user)」= **リクエストレベルのタイムアウト**で、
  内容不正・DOM 欠落ではない（＝キャッシュのバグではなく混雑由来）。
- スコアは混雑次第で 320k〜464k に振れる。CPU は依然 **非飽和**（mysql ~30-45% / rust ~40%）。

| 時刻 | 構成 | スコア | 備考 |
| --- | --- | --- | --- |
| 14:23 | + GET / メモリキャッシュ（混雑16台） | 319,738 | 混雑で抑制（fail 6594=タイムアウト） |
| 14:32 | 同上（混雑11台） | 342,011 | 同上 |

### 結論（500k への到達条件）
- 自己ベスト **464,134**。コードはレイテンシ律速まで最適化済み（GET / は DB 0 往復）。
- **500k はトップ群（Team4 543k / Team67 516k / Team77 492k）が到達済みで、混雑の少ない窓なら本コードでも到達可能なレンジ。**
  現状の高混雑下では 320k〜464k に抑制されており、これは共有ベンチマーカーの輻輳が主因（こちら側 CPU は非飽和）。
- 次にやるなら: 混雑の少ない時間帯での再計測 / GET /posts(ページング) もキャッシュ / `/@account` の軽量化。
