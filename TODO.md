...
最終更新: 2026-06-23 / **自己ベスト 464,134**（混雑時は320k〜、CPU非飽和） / 稼働言語 **Rust (`isu-rust`)**
...
### 1. GET / ・GET /posts のレイテンシ削減
- [x] **GET / の一覧データをメモリキャッシュ**（csrf は描画時に差し込み、書き込みで無効化）。
      → GET / は **DB 往復 0**。実行中 mysqld CPU 70%→30-45%。整合性は手動検証済み（コメント→即反映）。
- [ ] GET /posts（ページング, max_created_at）もキャッシュ or window 関数化（未着手）。
- [ ] `POSTS_FETCH_LIMIT`(現60) を 30〜40 に（小幅）。
